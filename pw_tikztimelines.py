...
def parse_tikz_comment(line, configs):
    """Search for tikz config lines within SQL comments. They start with TIKZ
    and are followed by a comma-separated list of attributes. We simply ignore
    incorrect TIKZ config lines, and assume that they are just normal SQL
    comments."""
    match = re.search(r'--\s*TIKZ:\s(.*)+?', line)
    if match:
        match = re.split(r'\s*,\s*', match.group(1))
        if match[0] == 'relation':
            cfg = dict(zip(['type', 'name', 'ts', 'te', 'desc'], match))
        elif match[0] == 'timeline':
            cfg = dict(zip(['type', 'from', 'to', 'desc'], match))
        configs.append(cfg)
        return match[0]
...
def parse_integrity_checks(configs_count, table_counts):
    """Integrity checks for already parsed PostgreSQL files with TIKZ comments.
    The following checks are made:
    1) Do we have enough config strings for all tables?
    2) Do we have zero or one timeline string, but not more?"""

    if configs_count['relation'] != table_counts:
        print_error_msg(
    ...
    for line in lines:
        ...
        if line.startswith('--'):
            ...
            if state == State.OUTSIDE:
                res = parse_tikz_comment(line, configs)
                configs_count[res] += 1
